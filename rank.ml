...
module RankFinder =
  struct
    ...
    module MyInt : MATRIX_TYPE with type t =  int =
    struct
      type t = int
      let plus = (+)
      let sub = (-)
    ...
    module IntMatrix = Matrix.Make(MyInt)

    module Z3m : MATRIX_TYPE with type t = Z3.ast =
    struct
      type t = Z3.ast
      let plus = add
      let sub = sub
    ...
    module Z3Matrix = Matrix.Make(Z3m)


    let make_var (name:string) =
      let sym = Z3.mk_string_symbol ctx name in
      Z3.mk_const ctx sym (Z3.mk_real_sort ctx)

    let make_const x = make_integer x

    let make_conjunction xs = Z3.mk_and ctx (Array.of_list xs)

    let make_constraint f v =
    ...
    let (=*) v k =
      make_constraint (fun x -> Z3.mk_eq ctx x (Z3.mk_int ctx k (Z3.mk_real_sort ctx))) v

    let (>=*) v k =
      make_constraint (fun x -> Z3.mk_ge ctx x (Z3.mk_int ctx k (Z3.mk_real_sort ctx))) v

    let (<*) v k =
      make_constraint (fun x -> Z3.mk_lt ctx x (Z3.mk_int ctx k (Z3.mk_real_sort ctx))) v
    ...
    let synthesis r =
      ...
      let vars = BatEnum.map
        (fun f -> make_var (Printf.sprintf "var%d" f)) naturals in
      ...
      let b = IntMatrix.scalar_multiply (-1) b in


      let a = IntMatrix.sub_left coefs 0 (r-1) 0 ((c-2)/2) in
      let a' = IntMatrix.sub_left coefs 0 (r-1) (((c-2)/2)+1) (c-2) in

      let b_symb = IntMatrix.map make_const b in
      let a_symb = IntMatrix.map make_const a  in
      let a_symb' = IntMatrix.map make_const a' in
      ...
      let c1 = (>=*) lambda1 0 in
      let c2 = (>=*) lambda2 0 in
      let c3 = (=*) (vector_multiply lambda1 (a_symb') ) 0 in
      let c4 = (<*) (vector_multiply lambda2 (b_symb) ) 0 in
      let c5 = (=*) (vector_multiply diff_lambda (a_symb) ) 0 in
      let c6 = (=*) (vector_multiply lambda2 (sum_a) ) 0 in

      let query = make_conjunction [c1; c2; c3; c4; c5; c6] in
